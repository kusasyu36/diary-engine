...
- 内向的だが「停滞」ではない。小さくとも今日という日に何かが動く（自分が一歩動く、
  世界の方から触れてくる、誰かと具体的に関わる）。内面描写は、その動きへの反応として書く。
...
- 大言壮語、自画自賛、話の飛躍が特徴。
- 【厳守】過去の栄光（特に「イランとの合意」）の蒸し返しと、それへの自賛・
  メディア批判の繰り返しを禁止。それはもう何十回も書いた。今日は『今この瞬間に
  進行している事態』だけを書く。
- 【厳守】今日は必ず、ハリントンの思い通りにならない要素を1つ起こす
  （部下の悪い報告／交渉相手の拒否や反撃／予想外の障害／支持率や市場の異変）。
  ハリントンはそれに反応し、対処しようとする。順風満帆な自慢だけの回にしない。
- たまに、ふと静かになる瞬間があり、虚勢の裏の本音がもれる。
